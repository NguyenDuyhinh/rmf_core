...
SCENARIO("Trajectory and base_iterator unit tests")
{
  ...
  GIVEN("Sample Trajectories")
  {
    ...
    param_inputs.push_back(
      {time, UnitBox, Eigen::Vector3d(0, 0, 0), Eigen::Vector3d(1, 1, 1)});
    param_inputs.push_back(
      {time + 10s,
        UnitBox, Eigen::Vector3d(2, 2, 2), Eigen::Vector3d(3, 3, 3)});
    param_inputs.push_back(
      {time + 20s,
        UnitBox, Eigen::Vector3d(4, 4, 4), Eigen::Vector3d(5, 5, 5)});
    rmf_traffic::Trajectory trajectory = create_test_trajectory(param_inputs);
    ...
    WHEN("Finding a waypoint at the precise time specified")
    {
      THEN("Waypoint is retreieved successfully")
      {
        ...
        CHECK(trajectory.find(time + 10s)->position() ==
          Eigen::Vector3d(2, 2, 2));
        CHECK(trajectory.find(time + 20s)->position() ==
    ...
    WHEN("Finding a waypoint at an offset time")
    {
      THEN("Waypoints currently active are retrieved successfully")
      {
        ...
        CHECK(trajectory.find(time + 8s)->position() ==
          Eigen::Vector3d(2, 2, 2));
        CHECK(trajectory.find(time + 12s)->position() ==
          Eigen::Vector3d(4, 4, 4));
        CHECK(trajectory.find(time + 20s)->position() ==
          Eigen::Vector3d(4, 4, 4));
      }
